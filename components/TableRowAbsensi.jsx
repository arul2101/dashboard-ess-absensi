...
export default function TableRowAbsensi({ date, clock_in, clock_out, location }) {
  const start = new Date(new Date(`${date} ${clock_in}`));
  const end = clock_out !== "-" ? new Date(new Date(`${date} ${clock_out}`)) : null;
  ...
  return (
    <TableRow>
      ...
      <TableCell className="text-center">{workedTime ? workedTime : "-"}</TableCell>
